...
Abl_trajectories = dataset.MDTrajDataset("/cbio/jclab/projects/fah/fah-data/munged2/no-solvent/11400/*.h5")
Src_trajectories = dataset.MDTrajDataset("/cbio/jclab/projects/fah/fah-data/munged2/no-solvent/11401/*.h5")
...
Abl_dihedral = np.load('Abl_dihedral_newsims.npy')
Src_dihedral = np.load('Src_dihedral_newsims.npy')
...
sns.distplot(Abl_rotate, color="r",label="Abl (35)")
sns.distplot(Src_rotate, color="b",label="Src (37)")
...
for i in range(len(Abl_line_rotate)):
	plt.axvline(Abl_line_rotate[i], color="r", ymin=0.95)
for i in range(len(Src_line_rotate)):
	plt.axvline(Src_line_rotate[i], color="b", ymin=0.95)

plt.xlabel('Dihedral (radians)')
plt.ylabel('Occupancy')
plt.legend()

plt.savefig('abl_src_DFG_dihedral_hist_newsims_lines_NYAS.png',dpi=1000)
