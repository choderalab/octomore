...
#Plot just vertical lines for Abl DFG

Abl_line_rotate_out = np.vstack([ Abl_line_rotate[index] for index in Abl_DFG_out[0]])
Abl_line_rotate_in = np.vstack([ Abl_line_rotate[index] for index in Abl_DFG_in[0]])

for i in range(len(Abl_line_rotate_in)):
        plt.axvline(Abl_line_rotate_in[i], color="m", ymin=0.95)
for i in range(len(Abl_line_rotate_out)):
        plt.axvline(Abl_line_rotate_out[i], color="r", ymin=0.95)

# Plot histogram with special seaborn sauce
sns.distplot(abl_in, color="m",label="Abl - in (%s) " %len(Abl_DFG_in[0]) )
sns.distplot(abl_out, color="r",label="Abl - out (%s) " %len(Abl_DFG_out[0]) )
...
plt.savefig('abl_DFG_dihedral_hist_newsims_NYAS.png',dpi=1000)
